...
export const CodeCompletion = ({ items, visible, selected }: CodeCompletionProps) => {
    ...
    <S.Container>
      <S.RowContainer>
        {items.map((item, index) => (
          <S.Row selected={selected === index} ref={(row: HTMLDivElement) => { itemsRef.current[index] = row }} key={item}>
            <span>{item}</span>
